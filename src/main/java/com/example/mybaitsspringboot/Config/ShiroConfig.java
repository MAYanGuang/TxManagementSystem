...
public class ShiroConfig {
	...
	public ShiroFilterFactoryBean getShiroFilterFactoryBean(DefaultWebSecurityManager defaultWebSecurityManager) {
		...
		//one接口有  update和add 的权限     登陆用户如果包含此权限则可以访问，否则不能访问
		fMap.put("/one", "perms[update,add]");
		// two接口有 add select delete update 的权限
		fMap.put("/two", "perms[add,select,delete,update]");
		//被拦截返回登录页面
